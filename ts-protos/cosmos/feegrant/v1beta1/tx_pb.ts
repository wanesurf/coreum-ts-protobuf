// Since: cosmos-sdk 0.43

// @generated by protoc-gen-es v2.0.0 with parameter "target=ts"
// @generated from file cosmos/feegrant/v1beta1/tx.proto (package cosmos.feegrant.v1beta1, syntax proto3)
/* eslint-disable */

import type { GenFile, GenMessage, GenService } from "@bufbuild/protobuf/codegenv1";
import { fileDesc, messageDesc, serviceDesc } from "@bufbuild/protobuf/codegenv1";
import type { Any } from "@bufbuild/protobuf/wkt";
import { file_google_protobuf_any } from "@bufbuild/protobuf/wkt";
import { file_cosmos_proto_cosmos } from "../../../cosmos_proto/cosmos_pb";
import { file_cosmos_msg_v1_msg } from "../../msg/v1/msg_pb";
import { file_amino_amino } from "../../../amino/amino_pb";
import type { Message } from "@bufbuild/protobuf";

/**
 * Describes the file cosmos/feegrant/v1beta1/tx.proto.
 */
export const file_cosmos_feegrant_v1beta1_tx: GenFile = /*@__PURE__*/
  fileDesc("", [file_google_protobuf_any, file_cosmos_proto_cosmos, file_cosmos_msg_v1_msg, file_amino_amino]);

/**
 * MsgGrantAllowance adds permission for Grantee to spend up to Allowance
 * of fees from the account of Granter.
 *
 * @generated from message cosmos.feegrant.v1beta1.MsgGrantAllowance
 */
export type MsgGrantAllowance = Message<"cosmos.feegrant.v1beta1.MsgGrantAllowance"> & {
  /**
   * granter is the address of the user granting an allowance of their funds.
   *
   * @generated from field: string granter = 1;
   */
  granter: string;

  /**
   * grantee is the address of the user being granted an allowance of another user's funds.
   *
   * @generated from field: string grantee = 2;
   */
  grantee: string;

  /**
   * allowance can be any of basic, periodic, allowed fee allowance.
   *
   * @generated from field: google.protobuf.Any allowance = 3;
   */
  allowance?: Any;
};

/**
 * Describes the message cosmos.feegrant.v1beta1.MsgGrantAllowance.
 * Use `create(MsgGrantAllowanceSchema)` to create a new message.
 */
export const MsgGrantAllowanceSchema: GenMessage<MsgGrantAllowance> = /*@__PURE__*/
  messageDesc(file_cosmos_feegrant_v1beta1_tx, 0);

/**
 * MsgGrantAllowanceResponse defines the Msg/GrantAllowanceResponse response type.
 *
 * @generated from message cosmos.feegrant.v1beta1.MsgGrantAllowanceResponse
 */
export type MsgGrantAllowanceResponse = Message<"cosmos.feegrant.v1beta1.MsgGrantAllowanceResponse"> & {
};

/**
 * Describes the message cosmos.feegrant.v1beta1.MsgGrantAllowanceResponse.
 * Use `create(MsgGrantAllowanceResponseSchema)` to create a new message.
 */
export const MsgGrantAllowanceResponseSchema: GenMessage<MsgGrantAllowanceResponse> = /*@__PURE__*/
  messageDesc(file_cosmos_feegrant_v1beta1_tx, 1);

/**
 * MsgRevokeAllowance removes any existing Allowance from Granter to Grantee.
 *
 * @generated from message cosmos.feegrant.v1beta1.MsgRevokeAllowance
 */
export type MsgRevokeAllowance = Message<"cosmos.feegrant.v1beta1.MsgRevokeAllowance"> & {
  /**
   * granter is the address of the user granting an allowance of their funds.
   *
   * @generated from field: string granter = 1;
   */
  granter: string;

  /**
   * grantee is the address of the user being granted an allowance of another user's funds.
   *
   * @generated from field: string grantee = 2;
   */
  grantee: string;
};

/**
 * Describes the message cosmos.feegrant.v1beta1.MsgRevokeAllowance.
 * Use `create(MsgRevokeAllowanceSchema)` to create a new message.
 */
export const MsgRevokeAllowanceSchema: GenMessage<MsgRevokeAllowance> = /*@__PURE__*/
  messageDesc(file_cosmos_feegrant_v1beta1_tx, 2);

/**
 * MsgRevokeAllowanceResponse defines the Msg/RevokeAllowanceResponse response type.
 *
 * @generated from message cosmos.feegrant.v1beta1.MsgRevokeAllowanceResponse
 */
export type MsgRevokeAllowanceResponse = Message<"cosmos.feegrant.v1beta1.MsgRevokeAllowanceResponse"> & {
};

/**
 * Describes the message cosmos.feegrant.v1beta1.MsgRevokeAllowanceResponse.
 * Use `create(MsgRevokeAllowanceResponseSchema)` to create a new message.
 */
export const MsgRevokeAllowanceResponseSchema: GenMessage<MsgRevokeAllowanceResponse> = /*@__PURE__*/
  messageDesc(file_cosmos_feegrant_v1beta1_tx, 3);

/**
 * MsgPruneAllowances prunes expired fee allowances.
 *
 * Since cosmos-sdk 0.50
 *
 * @generated from message cosmos.feegrant.v1beta1.MsgPruneAllowances
 */
export type MsgPruneAllowances = Message<"cosmos.feegrant.v1beta1.MsgPruneAllowances"> & {
  /**
   * pruner is the address of the user pruning expired allowances.
   *
   * @generated from field: string pruner = 1;
   */
  pruner: string;
};

/**
 * Describes the message cosmos.feegrant.v1beta1.MsgPruneAllowances.
 * Use `create(MsgPruneAllowancesSchema)` to create a new message.
 */
export const MsgPruneAllowancesSchema: GenMessage<MsgPruneAllowances> = /*@__PURE__*/
  messageDesc(file_cosmos_feegrant_v1beta1_tx, 4);

/**
 * MsgPruneAllowancesResponse defines the Msg/PruneAllowancesResponse response type.
 *
 * Since cosmos-sdk 0.50
 *
 * @generated from message cosmos.feegrant.v1beta1.MsgPruneAllowancesResponse
 */
export type MsgPruneAllowancesResponse = Message<"cosmos.feegrant.v1beta1.MsgPruneAllowancesResponse"> & {
};

/**
 * Describes the message cosmos.feegrant.v1beta1.MsgPruneAllowancesResponse.
 * Use `create(MsgPruneAllowancesResponseSchema)` to create a new message.
 */
export const MsgPruneAllowancesResponseSchema: GenMessage<MsgPruneAllowancesResponse> = /*@__PURE__*/
  messageDesc(file_cosmos_feegrant_v1beta1_tx, 5);

/**
 * Msg defines the feegrant msg service.
 *
 * @generated from service cosmos.feegrant.v1beta1.Msg
 */
export const Msg: GenService<{
  /**
   * GrantAllowance grants fee allowance to the grantee on the granter's
   * account with the provided expiration time.
   *
   * @generated from rpc cosmos.feegrant.v1beta1.Msg.GrantAllowance
   */
  grantAllowance: {
    methodKind: "unary";
    input: typeof MsgGrantAllowanceSchema;
    output: typeof MsgGrantAllowanceResponseSchema;
  },
  /**
   * RevokeAllowance revokes any fee allowance of granter's account that
   * has been granted to the grantee.
   *
   * @generated from rpc cosmos.feegrant.v1beta1.Msg.RevokeAllowance
   */
  revokeAllowance: {
    methodKind: "unary";
    input: typeof MsgRevokeAllowanceSchema;
    output: typeof MsgRevokeAllowanceResponseSchema;
  },
  /**
   * PruneAllowances prunes expired fee allowances, currently up to 75 at a time.
   *
   * Since cosmos-sdk 0.50
   *
   * @generated from rpc cosmos.feegrant.v1beta1.Msg.PruneAllowances
   */
  pruneAllowances: {
    methodKind: "unary";
    input: typeof MsgPruneAllowancesSchema;
    output: typeof MsgPruneAllowancesResponseSchema;
  },
}> = /*@__PURE__*/
  serviceDesc(file_cosmos_feegrant_v1beta1_tx, 0);

